...
Six algorithms were trained per problem, chosen to span different learning styles:
linear, instance based, a single tree, a bagged ensemble, a boosted ensemble and a
margin based model.
...
Decision Tree                  & 0.970 & 0.948 & 0.9510 & 0.794 & 0.022 \\
Random Forest                  & 0.964 & 0.939 & 0.9500 & 0.818 & 0.025 \\
...
Decision Tree Regressor       & 8.35 & 8.71 & 8.56 & 13.86 & 0.925 \\
Random Forest Regressor       & 6.85 & 7.91 & 7.79 & 11.06 & 0.952 \\
...
Four hyperparameter sweeps were run, and all four produced the same shape: validation
...
$k=50$ \\
Decision Tree & \texttt{max\_depth} &
Validation peaks at depth 4 (0.943) then falls while train climbs to 0.984 unlimited. &
depth 6 \\
...
of all six models, trains in a fraction of a second, and produces directly interpretable
...
of all six models, beating every more complex alternative outright. This is coherent
...
\textbf{Clustering.} K-Means and a Gaussian Mixture Model were applied to six flight
shape features (\texttt{CRSElapsedTime}, \texttt{Distance}, \texttt{dep\_sin},
\texttt{dep\_cos}, \texttt{DepDelay}, \texttt{TaxiOut}), standardised because both
methods are scale sensitive. \textbf{The target was never used.}
...
The GMM ($k{=}4$, chosen to match for comparison since BIC decreases monotonically
through $k{=}8$) independently found a comparable group: cluster 1 (2,755 flights) has
mean \texttt{TaxiOut} 30.7 minutes, the highest by far, mean \texttt{ArrDelay} 103.6 and
92\% delayed. The two algorithms agree that a distinct, severely delayed segment exists,
but define its boundary differently: K-Means anchors on total delay magnitude, GMM on
ground congestion. Silhouette values of 0.25 to 0.32 are modest, as expected for
continuous operational data with no natural gaps between groups.
